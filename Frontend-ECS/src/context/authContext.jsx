...
export const AuthProvider = ({ children }) => {
  ...
  const [isECE, setIsECE] = useState(() => {
    return localStorage.getItem("isECE") === "true";
  });
  ...
  useEffect(() => {
    localStorage.setItem("isECE", isECE);
  }, [isECE]);

  return (
    <AuthContext.Provider value={{ isLoggedIn, setIsLoggedIn, isECE, setIsECE }}>
      {children}
    </AuthContext.Provider>
  );
};
